...
public class WordOfTheDayService
{
	...
	private static object _wotdLock = new();
	private static object _addWordLock = new();
	private static object _deleteWordLock = new();
	...
	public async Task<bool> DeleteWord(string word)
	{
		...
		{
			lock (_deleteWordLock)
			{
				foundWord = Db.Words.FirstOrDefault(dbWord => dbWord.Text.Equals(word));
				if (foundWord is not null)
				{
					Db.Words.Remove(foundWord);
					Db.SaveChanges();
					return true;
				}
				return false;
