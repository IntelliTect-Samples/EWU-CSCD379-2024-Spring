<template>
    <v-row class="justify-center" v-for="(keyboardRow, rowIndex) in keyboardLetterRows" :key="rowIndex" dense>
      <v-col cols="auto" v-for="letter in keyboardRow" :key="letter.char" :class="'ml-1 px-0'">
        <LetterResult :letter="letter" :clickable="true" :is-keyboard="true" @click="handleLetterClick(letter)" />
      </v-col>
    </v-row>
  </template>
    ...
    for (let keyboardRow of keyboardRows) {
      ...
      for (let key of keyboardRow) {
        keyboardLetterRow.push(
          game.guessedLetters.find((letter) => letter.char === key) ?? new Letter(key)
        );
      }
