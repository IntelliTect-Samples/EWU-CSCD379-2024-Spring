    ...
    <v-card v-else class="text-center" elevation="2">
      <v-sheet>
        <v-container height="150px">
          <v-sheet
            @click="toggleNameDialog"  <!-- Method to toggle the dialog -->
            class="pa-2 mx-2 mt-2 cursor-pointer"
            color="deep-purple accent-4">
            {{ username || 'Click to set your name' }} <!-- Placeholder if username is empty -->
          </v-sheet>
        </v-container>
      </v-sheet>
      <v-alert
        ...
        tile>
        <h3>
          You've {{ game.gameState === GameState.Won ? 'triumphed! 🏰' : 'been defeated... 🌑' }}
        </h3>
        <v-card-text>
          The secret word was: <strong>{{ game.secretWord }}</strong>
        </v-card-text>
        <v-btn text color="blue" @click="game.startNewGame()">
      ...
      <v-btn @click="$router.push('/leaderboard'); playAudio();" class="mb-5 mx-5" color="deep-purple darken-2">
        View Hall of Heroes
      </v-btn>
      <NameDialog
        v-model:show="showNameDialog"
        :name="username"
        @update:name="username = $event"  <!-- Update username on change -->
        @entered="enterName" />
      <GuestSaveDialog
...
const showNameDialog = ref(!username.value || username.value === 'Guest');
